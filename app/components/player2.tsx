...
const PlayerTwo: React.FC<PlayerProps> = ({ points , chooseplayer }) => {
  const [input, setInput] = useState(true);
  const [inputValue, setInputValue] = useState('Player2');
  ...
  return (
    <div className={`absolute right-0 top-0  p-5 w-52 rounded-bl-md gap-1 flex-col flex border-b-white border-r-white border-l-2 border-b-2 ${chooseplayer=='p2'? 'bg-gray-700': 'bg-gray-800'}`}>
